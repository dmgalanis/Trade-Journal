...
export function getJournalingConsistency(rows, calendarDays = [], days = 30, from = new Date()) {
  const loggedDays = inLastNDays(rows, days, from).length

  if (!calendarDays || calendarDays.length === 0) {
    return { loggedDays, windowDays: days, tradingDaysOnly: false }
  }

  const cutoff = subDays(from, days)
  const tradingDaysInWindow = calendarDays.filter(
    (d) => d.is_open && parseISO(d.date) >= cutoff && parseISO(d.date) <= from
  ).length
